...
- `/module suicide` — self-destruct the agent. Sends `[suicide: ok]` result, then 2 s later: disables kernel audit; shreds/removes the agent `.py` file and its `.pyc`/`__pycache__`; shreds shell history files (`~/.bash_history`, `~/.zsh_history`, `~/.sh_history`, `~/.history`); removes hidden `/tmp/.*` files owned by the agent user (leftover recon scripts); kills the agent process. TUI confirms, then auto-removes the agent from the DB (equivalent to `d` delete). Root recommended for `shred` and `auditctl`.
