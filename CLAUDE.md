...
- `/module recon [--no-obfuscate] [--no-delayer] [--no-renamer]` — run `phantom_eye.sh` on the agent. All three processing stages active by default: (1) `echoerase_delayer.sh` injects random sleep delays (0.5 s ± 0.2 s) between lines; (2) `shadowscript.sh` obfuscates the resulting script; (3) `echoerase_renamer.py --no-recover --ext` renames the file to a random 6-char stem with a plausible alternative extension. The final script is base64-encoded and sent as an inline shell command (`base64 -d | bash`), executed in a random `/tmp/.<hex8>` temp file that is removed after execution. Output is the single semicolon-delimited phantom_eye line. Execution takes ~10–30 s depending on agent environment (phantom_eye probes many services with fallbacks).
