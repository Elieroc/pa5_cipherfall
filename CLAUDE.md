...
**Privesc** (`dirtyfrag/`, `ssh-keysign-pwn/`, `fragnesia.sh`, `copyfail.py`): Four exploits. `dirtyfrag` targets CVE via fragmented memory. `ssh-keysign-pwn` abuses `ssh-keysign` SUID binary (also includes `chage_pwn.c` and `exploit_vuln_target.c`). `fragnesia.sh` escalates via user+network namespace (CVE-2026-46300) using `unshare --user --map-root-user --net`; outputs `[privesc:ok] fragnesia` + `id` (uid=0, namespace-scoped); when invoked via TUI `/module privesc fragnesia`, the C2 uploads the script via WRITE:, executes it, then spawns a root agent inside the namespace using `unshare --user --map-root-user --net -- bash -c '...'` as the escalation primitive (namespace root only, not host root). `copyfail.py` implements splice-based arbitrary-write via AF_ALG + KTLS socket (ctypes `libc.splice()`), overwrites `/bin/su` in-place kernel-copy-style, then calls `os.system("su")`; requires Python ≥ 3.10.
